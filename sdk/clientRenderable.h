...
public:
	...
	// Data accessors
	virtual int const& GetRenderOrigin(void) = 0;
	virtual int const& GetRenderAngles(void) = 0;
	...
	// Render baby!
	virtual const int* GetModel() const = 0;
	...
	virtual bool	SetupBones(mat3x4* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime) = 0;

	virtual void	SetupWeights(const mat3x4* pBoneToWorld, int nFlexWeightCount, float* pFlexWeights, float* pFlexDelayedWeights) = 0;
	...
	virtual bool	GetShadowCastDistance(float* pDist, Vector3 shadowType) const = 0;
	virtual bool	GetShadowCastDirection(Vector3* pDirection, Vector3 shadowType) const = 0;
	...
	virtual const Vector3& RenderableToWorldTransform() = 0;

	// Attachments
	virtual int LookupAttachment(const char* pAttachmentName) = 0;
	virtual	bool GetAttachment(int number, Vector3& origin, Vector3& angles) = 0;
	virtual bool GetAttachment(int number, Vector3& matrix) = 0;
...
inline IClientRenderable* clientRenderable = create